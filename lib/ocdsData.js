...
export function tenderObject(contract) {
  ...
  const document = {
    ...
    // awardCriteriaDetailsScaleDetailsMxCnet: contract.ESTRATIFICACION_MUC,
    contractPeriod: {
      multiyearContractMxCnet: contract.PLURIANUAL,
    },
    submissionMethod: contract.FORMA_PROCEDIMIENTO,
  };
  ...
  //   Object.assign(document, {
  //     awardCriteriaDetailsScale: tenderAwardCriteriaDetailScale(contract.ESTRATIFICACION_MUC),
  //   })
  // }
...
function contractAmendmentsObject(convenio) {
    if(convenio) {
        return { id: convenio };
    }
    return null;
...
export function contractObject(contract) {
  ...
  const contractObj = {
    ...
    awardID: 1, // FIXME overwrite on import: award/id,
    period: {
    ...
    // },
    dateSigned: date,
    hasFramework: contract.CONTRATO_MARCO ? true : false,
    framework: contract.IDENTIFICADOR_CM,
    filedMxCnet: contract.ARCHIVADO ? true : false,
  };

  if(contract.CONVENIO_MODIFICATORIO == 1) {
      Object.assign(contractObj, {
          amendments: contractAmendmentsObject(contract.CONVENIO_MODIFICATORIO)
      })
  }
...
export function supplierPartyObject(contract) {
  ...
  if (contract.ESTRATIFICACION_MUC) {
    Object.assign(details, {
        scaleReportedByBuyerMxCnet: contract.ESTRATIFICACION_MUC,
    })
  }
...
export function buyerPartyObject(contract) {
  ...
  // ESTO YA NO! Lo dejo por si acaso. F
  // if (contract.ESTRATIFICACION_MPC) {
  //   Object.assign(party, {
  //     details: {
